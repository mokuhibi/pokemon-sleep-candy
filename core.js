...
const CandyCore = (() => {
 ...
 const profileValid=(p,species)=>p&&Number.isInteger(p.level)&&p.level>=1&&p.level<=100&&typeof p.nature==='string'&&p.nature.length>0&&(species!=='ミュウ'||p.nature==='きまぐれ')&&Number.isInteger(p.skillLevel)&&p.skillLevel>=1&&p.skillLevel<=8&&Array.isArray(p.subskills)&&p.subskills.length===5&&p.subskills.every(x=>typeof x==='string')&&new Set(p.subskills.filter(Boolean)).size===p.subskills.filter(Boolean).length;
 ...
 function migrate(raw,map){
  ...
  const ids=new Set();
  for(const p of raw.pokemon){if(!p||typeof p.id!=='string'||ids.has(p.id)||!Object.hasOwn(map,p.species)||typeof p.nickname!=='string'||(p.profile!=null&&!profileValid(p.profile,p.species)))throw Error('個体情報');ids.add(p.id);}
  ...
  const rids=new Set();for(const r of raw.records){
   ...
   if(r.context!=null){const c=r.context;if(!Array.isArray(c.team)||c.team.length!==5)throw Error('記録時編成');
    for(const p of c.team)if(p!==null&&(!p||typeof p.id!=='string'||typeof p.nickname!=='string'||!Object.hasOwn(map,p.species)||(p.profile!=null&&!profileValid(p.profile,p.species))))throw Error('記録時個体');
    if(r.method!=='help'){if(!Number.isInteger(c.actorSlot)||c.actorSlot<1||c.actorSlot>5||!c.team[c.actorSlot-1]||c.team[c.actorSlot-1].id!==c.actorId||c.team[c.actorSlot-1].species!==(r.method==='mew'?'ミュウ':'デリバード')||!profileValid(c.team[c.actorSlot-1].profile,c.team[c.actorSlot-1].species)||!c.event||![1,1.25,1.5].includes(c.event.multiplier)||!Number.isInteger(c.event.boost)||c.event.boost<0||c.event.boost>7||c.effectiveLevel!==Math.min(8,c.team[c.actorSlot-1].profile.skillLevel+c.event.boost))throw Error('発動個体');if(r.method==='mew'&&!mewAmounts(c.effectiveLevel).includes(r.amount))throw Error('スキル個数');}
   }
  }
  ...
  return {version:2,pokemon:raw.pokemon.map(p=>({...p,profile:p.profile||null})),team:raw.team,records:raw.records.map(r=>({...r,context:r.context||null})),events,settings:raw.settings||{mew:true,delibird:true}};
 }
 const sum=rs=>rs.reduce((n,r)=>n+r.amount,0);
 return {gameDay,localInput,fromInput,dateOnly,addDays,range,inRange,mewAmounts,profileDefault,profileValid,eventFor,migrate,sum};
})();
