...
const WeeklyChart={
 render(root,records,day){
  ...
  days.forEach((d,i)=>{const col=el('div',undefined,'weekly-column'),value=el('span',d.amount.toLocaleString('ja-JP'),'weekly-value'),track=el('div',undefined,'weekly-track'),bar=el('div',undefined,'weekly-bar');bar.style.height=(d.amount/max*100)+'%';track.append(bar);col.append(value,track,el('strong',C.weekday(d.date),'weekly-weekday'),el('span',Number(d.date.slice(5,7))+'/'+Number(d.date.slice(8,10)),'weekly-date'));col.title=d.date+'：'+d.amount.toLocaleString('ja-JP')+'個';chart.append(col);});root.append(chart);
 }
};
