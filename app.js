...
function renderHistory(){const today=C.gameDay(new Date());$('history-day').textContent=showPastHistory?'全期間':today+'の履歴';$('history-toggle').textContent=showPastHistory?'当日の履歴だけ表示':'過去の履歴を表示';const root=$('history-list');root.replaceChildren();for(const r of [...state.records].filter(r=>visibleRecord(r)&&(showPastHistory||C.gameDay(r.datetime)===today)).reverse().sort((a,b)=>Date.parse(b.datetime)-Date.parse(a.datetime))){
 const card=el('div',undefined,'card '+r.method),head=el('div',undefined,'history-head'),actions=el('div',undefined,'history-actions');
 ...
 }if(!root.children.length)root.append(el('p','履歴なし'));}
...
function showTab(id){activeTab=id;for(const name of ['record','summary','history','team','settings'])$(name).hidden=name!==id;for(const b of document.querySelectorAll('[data-tab]')){if(b.dataset.tab===id)b.setAttribute('aria-current','page');else b.removeAttribute('aria-current');}if(id==='record')renderRecord();if(id==='summary')renderSummary();if(id==='history')renderHistory();window.scrollTo({top:0});}
function render(){renderRecord();renderActorFilters();renderSummary();renderHistory();renderTeam();renderSettings();renderCatalog();for(const o of $('event-target').options){if(o.value!=='both')o.hidden=!enabled(o.value);}if($('event-target').value!=='both'&&!enabled($('event-target').value))$('event-target').value='both';$('event-target').options[0].textContent=['mew','delibird'].filter(enabled).map(m=>methods[m]).join('・');}
...
$('add-pokemon').onclick=()=>{if(!names.includes(selected)||!speciesEnabled(selected))return;if(selected==='ミュウ'&&state.pokemon.some(p=>p.species==='ミュウ')){notice('ミュウは登録済みです。');return;}const p={id:uid(),species:selected,nickname:$('nickname').value.trim(),profile:null,registrationOrder:Math.max(-1,...state.pokemon.map(p=>p.registrationOrder))+1};if(commit({...state,pokemon:[...state.pokemon,p]},label(p)+'を登録しました。')){$('nickname').value='';if(['ミュウ','デリバード'].includes(p.species)){showTab('settings');$('profile-select').value=p.id;editProfile();notice(label(p)+'を登録しました。');}}};
...
$('import').onchange=async()=>{const file=$('import').files[0];if(!file)return;try{const data=CandyBackup.read(await file.text());const next=C.migrate(data,candyMap);if(confirm(`登録${next.pokemon.length}匹・履歴${next.records.length}件で現在のデータを置き換えますか？`)){localStorage.setItem(KEY+'-before-import',localStorage.getItem(KEY)||'null');const wasBlocked=blocked;blocked=false;if(!commit(next,'バックアップを復元しました。'))blocked=wasBlocked;}}catch(e){notice('復元できませんでした：'+e.message);}$('import').value='';};
