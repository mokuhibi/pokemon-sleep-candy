...
function renderCatalog(){const q=normalize($('pokemon-search').value),root=$('pokemon-search-results');root.replaceChildren();for(const name of names.filter(n=>speciesEnabled(n)&&normalize(n).includes(q))){const b=button(name,()=>{selected=name;$('selected-name').textContent=name;$('add-pokemon').disabled=false;});b.disabled=name==='ミュウ'&&state.pokemon.some(p=>p.species==='ミュウ');root.append(b);}if(!root.children.length)root.append(el('p','該当するポケモンがありません。'));}
...
$('import').onchange=async()=>{const file=$('import').files[0];if(!file)return;try{const text=(await file.text()).replace(/^\uFEFF/,'');const data=text.trimStart().startsWith('{')?JSON.parse(text):CandyBackup.decode(text);const next=C.migrate(data,candyMap);if(confirm(`登録${next.pokemon.length}匹・履歴${next.records.length}件で現在のデータを置き換えますか？`)){localStorage.setItem(KEY+'-before-import',localStorage.getItem(KEY)||'null');const wasBlocked=blocked;blocked=false;if(!commit(next,'バックアップを復元しました。'))blocked=wasBlocked;}}catch(e){notice('復元できませんでした：'+e.message);}$('import').value='';};
