...
let noticeTimer;
function notice(s){const n=$('notice');quantityText(n,s);n.classList.remove('shown');void n.offsetWidth;n.classList.add('shown');clearTimeout(noticeTimer);noticeTimer=setTimeout(()=>n.classList.remove('shown'),5000);}
...
function quantityText(node,text){
 node.replaceChildren();const re=/(\d+(?:\.\d+)?)(個|回)/g;let end=0;
 for(const m of String(text).matchAll(re)){node.append(document.createTextNode(String(text).slice(end,m.index)));const q=el('span',undefined,'quantity');q.append(el('span',m[1],'quantity-number'),el('span',m[2],'quantity-unit'));node.append(q);end=m.index+m[0].length;}
...
function skillContext(method){const team=currentTeam(),actorIndex=team.findIndex(p=>p&&p.id===actors[method]);const p=method==='mew'?state.pokemon.find(p=>p.species==='ミュウ'):team[actorIndex];const date=recordDate();if(!p||!p.profile||!Number.isFinite(date.getTime()))return null;const event=C.eventFor(state.events,C.gameDay(date),method);return {rulesVersion:3,team:clone(team),actor:clone(p),actorId:p.id,actorSlot:actorIndex>=0?actorIndex+1:null,event,effectiveLevel:Math.min(C.skillCap(p.species),p.profile.skillLevel+event.boost)};}
...
function record(method,slot,amount){const date=recordDate();if(!Number.isFinite(date.getTime())){notice('記録日時を入力してください。');return;}const team=currentTeam(),p=team[slot-1],skillOnly=method==='delibird'&&slot===6;const context=method==='help'?{team:clone(team),actorId:null,actorSlot:null,event:null,effectiveLevel:null}:skillContext(method);
 ...
 const r={id:uid(),datetime:date.toISOString(),method,amount,slot,pokemonId:skillOnly?null:p.id,pokemon:skillOnly?'スキルのみ':label(p),species:skillOnly?null:p.species,nickname:skillOnly?'':p.nickname,candy:skillOnly?null:candyMap[p.species],context};
 commit({...state,records:[...state.records,r]},`${methods[method]}：${skillOnly?'スキルのみ（アメなし）':position(slot)+'・'+r.candy+' '+amount+'個'}を記録しました。`);
}
function totals(parent,records,title){records=records.filter(visibleRecord);parent.replaceChildren(el('small',title),qel('div',`${C.sum(records)}個`,'total'));for(const [method,name] of Object.entries(methods)){if(!enabled(method))continue;const rs=records.filter(r=>r.method===method);row(parent,name,method==='help'?`${C.sum(rs)}個`:`スキル${rs.length}回 · アメ${C.sum(rs)}個`,method);}}

function renderRecord(){
 updateFixedRecordDate();
 ShardUI.renderInputs();
 totals($('today-total'),state.records.filter(r=>C.gameDay(r.datetime)===C.gameDay(new Date())),`今日 ${C.gameDay(new Date())}`);
 ...
 for(const [method,name] of Object.entries(methods)){
  ...
  team.forEach((p,i)=>{
   ...
   if(method==='mew'){const actions=el('div',undefined,'mew-actions');for(const amount of context?C.mewAmounts(context.effectiveLevel):[1]){const b=button('',()=>record(method,i+1,amount));quantityText(b,amount+'個');b.disabled=!p||!context;actions.append(b);}box.append(actions);}
...
function renderActorFilters(){
 const s=$('summary-actor');optionList(s,[...historicalActors('delibird'),{id:'unknown',text:'スキル個体不明'}],s.value,'全個体');s.closest('label').hidden=!enabled('delibird');
...
function renderSummary(){
 ...
 $('slot-counts').replaceChildren();for(const [method,name] of Object.entries(methods)){if(!enabled(method))continue;const group=el('div',undefined,method);const source=rs.filter(r=>r.method===method);group.append(qel('h4',`${name}${method==='help'?'':` · スキル${source.length}回`}`));for(let i=1;i<=5;i++){const xs=source.filter(r=>r.slot===i);row(group,position(i),`${xs.length}回 · ${C.sum(xs)}個`);}if(method==='delibird')row(group,'6 · スキルのみ',source.filter(r=>r.slot===6).length+'回');const unknown=source.filter(r=>r.slot===null);if(unknown.length)row(group,'獲得位置不明',unknown.length+'回 · '+C.sum(unknown)+'個');$('slot-counts').append(group);}
...
function renderAnalysis(){
 ...
 const source=records.filter(r=>r.method===method),unknown=source.filter(r=>!r.context?.actorId||!r.context.actorSlot||!r.slot).length;
 const rs=source.filter(r=>r.context?.actorId&&r.context.actorSlot&&r.slot&&(!actor||r.context.actorId===actor)&&(!level||r.context.effectiveLevel===Number(level))&&(!multi||r.context.event.multiplier===Number(multi))&&(amount===''||r.amount===Number(amount)));
 const root=$('analysis');root.className=method;root.replaceChildren(qel('p',`分析対象 ${rs.length}回 · 位置などが不明な記録 ${unknown}回は除外`));
 const wrap=el('div',undefined,'table-scroll'),table=el('table'),head=el('tr');table.className='position-table';const caption=el('caption','獲得先 →');table.append(caption);head.append(el('th','スキル位置 ↓'));for(let i=1;i<=(method==='mew'?5:6);i++)head.append(el('th',i===6?'アメなし':String(i)));head.append(el('th','スキル回数'));table.append(head);
 for(let i=1;i<=5;i++){const tr=el('tr'),xs=rs.filter(r=>r.context.actorSlot===i);tr.append(el('th',i===1?'1\nリーダー':String(i)));for(let j=1;j<=(method==='mew'?5:6);j++){const count=xs.filter(r=>r.slot===j).length;tr.append(qel('td',`${count}回\n${xs.length?(count/xs.length*100).toFixed(1)+'%':'—'}`));}tr.append(qel('td',xs.length+'回'));table.append(tr);}wrap.append(table);root.append(wrap);
...
function oldLabel(r){if(r.amount===0)return 'スキルのみ';if(typeof r.nickname==='string')return r.nickname||r.species;return r.pokemon.replace(/ · #[a-zA-Z0-9-]+$/,'').replace('（'+r.species+'）','');}
...
function renderHistory(){const today=C.gameDay(new Date());$('history-day').textContent=showPastHistory?'全期間':today+'の履歴';$('history-toggle').textContent=showPastHistory?'当日の履歴だけ表示':'過去の履歴を表示';const root=$('history-list');root.replaceChildren();for(const r of [...state.records].filter(r=>visibleRecord(r)&&(showPastHistory||C.gameDay(r.datetime)===today)).reverse().sort((a,b)=>Date.parse(b.datetime)-Date.parse(a.datetime))){
 ...
 head.append(qel('strong',`${methods[r.method]} · ${r.amount}個`),actions);card.append(head,el('p',`${C.localInput(r.datetime).replace('T',' ')}\n${r.slot===6?'アメなし':r.slot?position(r.slot):'獲得位置不明'} · ${r.candy||'スキルのみ'}\n${oldLabel(r)}`));const details=el('details');details.append(el('summary',r.method==='help'?'編成':'編成・スキル'));
 if(!r.context)details.append(el('p',r.method==='help'?'編成不明':'編成・スキル個体不明'));
 else{const c=r.context;if(r.method!=='help'){const p=actorOf(c);details.append(el('p',`スキル：${actorPosition(c)} · ${label(p)}\nLv.${c.effectiveLevel} · ${c.event.multiplier}倍 · レベル+${c.event.boost} · ${c.event.name}`));}c.team.forEach((p,i)=>{if(p&&!speciesEnabled(p.species))return;details.append(el('p',`${position(i+1)} · ${p?label(p):'未設定'}${p&&special(p)?'\n'+profileText(p):''}`));});}card.append(details);root.append(card);
 }ShardUI.appendHistory(root,today,showPastHistory);if(!root.children.length)root.append(el('p','履歴なし'));}
...
function renderTeam(){const root=$('team-slots');root.replaceChildren();state.team.forEach((id,i)=>{const l=el('label',position(i+1)),s=el('select');s.setAttribute('aria-label',position(i+1));const current=state.pokemon.find(p=>p.id===id);if(current&&!speciesEnabled(current.species)){s.add(new Option('非表示',id));s.disabled=true;}else{optionList(s,orderedPokemon().filter(p=>speciesEnabled(p.species)).map((p,n)=>({id:p.id,text:`${label(p)}${state.pokemon.filter(x=>label(x)===label(p)).length>1?' · 個体'+(n+1):''}`})),id,'未設定');for(const o of s.options)if(o.value&&state.team.some((pid,j)=>j!==i&&pid===o.value))o.disabled=true;s.onchange=()=>{const team=[...state.team];team[i]=s.value||null;commit({...state,team},'編成を保存しました。');};}l.append(s);root.append(l);});
 ...
 const list=$('registered-pokemon-list');list.replaceChildren();const normal=state.pokemon.filter(p=>!special(p));for(const p of orderedPokemon().filter(p=>speciesEnabled(p.species))){const card=el('div',undefined,'registered-row '+(p.species==='ミュウ'?'mew':p.species==='デリバード'?'delibird':'')),actions=el('div',undefined,'row-actions');card.append(el('strong',label(p)));if(sorting&&!special(p)){const i=normal.findIndex(x=>x.id===p.id),up=button('↑',()=>movePokemon(p.id,-1)),down=button('↓',()=>movePokemon(p.id,1));up.setAttribute('aria-label',label(p)+'を上へ');down.setAttribute('aria-label',label(p)+'を下へ');up.disabled=i===0;down.disabled=i===normal.length-1;actions.append(up,down);}if(special(p))actions.append(button('編集',()=>{showTab('settings');$('profile-select').value=p.id;editProfile();}));actions.append(button('登録解除',()=>{if(confirm(label(p)+'を登録解除しますか？'))commit({...state,pokemon:state.pokemon.filter(x=>x.id!==p.id),team:state.team.map(id=>id===p.id?null:id)},'登録解除しました。');},'danger'));card.append(actions);list.append(card);ShardUI.addSkillSetting(card,p);}}

function normalize(s){return s.normalize('NFKC').trim().replace(/[\u3041-\u3096]/g,c=>String.fromCharCode(c.charCodeAt(0)+96));}
function renderCatalog(){const q=normalize($('pokemon-search').value),root=$('pokemon-search-results');root.replaceChildren();root.hidden=!!selected;if(selected)return;for(const name of names.filter(n=>speciesEnabled(n)&&normalize(n).includes(q))){const b=button(name,()=>{selected=name;$('pokemon-search').value=name;$('selected-name').textContent=name;$('add-pokemon').disabled=false;root.hidden=true;});b.disabled=name==='ミュウ'&&state.pokemon.some(p=>p.species==='ミュウ');root.append(b);}if(!root.children.length)root.append(el('p','該当するポケモンがありません。'));}
...
function renderSettings(){ $('enable-mew').checked=state.settings.mew;$('enable-delibird').checked=state.settings.delibird;const p=$('profile-select'),old=p.value;optionList(p,orderedPokemon().filter(p=>special(p)&&speciesEnabled(p.species)).map((p,i)=>({id:p.id,text:p.species==='ミュウ'?label(p):`${label(p)} · 個体${orderedPokemon().filter(x=>x.species==='デリバード').findIndex(x=>x.id===p.id)+1}`})),old);editProfile();$('profile-select').closest('.card').hidden=!enabled('mew')&&!enabled('delibird');$('event-month').closest('.card').hidden=!enabled('mew')&&!enabled('delibird');renderEvents();}
...
function openHistoryEditor(r){
 ...
 const species=$('edit-species');species.replaceChildren();for(const name of names.filter(speciesEnabled))species.add(new Option(name,name));species.value=r.species||names[0];
 $('edit-context').checked=!!r.context;const root=$('edit-team');root.replaceChildren();
 for(let i=0;i<5;i++){const l=el('label',position(i+1)),s=el('select');s.id='edit-team-'+i;optionList(s,editPool.map((p,n)=>({id:p.id,text:speciesEnabled(p.species)?`${label(p)} · 個体${n+1}`:'非表示'})),r.context?.team[i]?.id||'','未設定');s.onchange=()=>refreshHistoryEditor();l.append(s);root.append(l);}
 $('edit-multiplier').value=r.context?.event?.multiplier||1;$('edit-boost').value=r.context?.event?.boost||0;
 refreshHistoryEditor(r.slot,r.amount,r.context?.actorId||'');$('history-editor').showModal();
}
...
function refreshHistoryEditor(slot,amount,actorId){
 const method=$('edit-method').value,s=$('edit-slot'),oldSlot=slot===undefined?s.value:String(slot??'');s.replaceChildren(new Option('不明',''));for(let i=1;i<=(method==='delibird'?6:5);i++)s.add(new Option(i===6?'6 · アメなし':position(i),String(i)));s.value=oldSlot;
 ...
 const values=method==='help'?[2]:method==='delibird'?s.value==='6'?[0]:[4]:$('edit-context').checked&&lv?C.mewAmounts(lv):[1,2,3,4];
 const a=$('edit-amount'),oldAmount=amount===undefined?Number(a.value):amount;a.replaceChildren();for(const n of values)a.add(new Option(n+'個',String(n)));a.value=values.includes(oldAmount)?String(oldAmount):String(values[0]);
 const p=team[Number(s.value)-1];if($('edit-context').checked&&p)$('edit-species').value=p.species;
 $('edit-species-label').hidden=s.value==='6';$('edit-species').disabled=$('edit-context').checked;
 ...
 actor.disabled=!$('edit-context').checked;
}
$('edit-cancel').onclick=()=>$('history-editor').close();
for(const id of ['edit-method','edit-slot','edit-actor','edit-boost','edit-context'])$(id).onchange=()=>refreshHistoryEditor();
...
$('history-edit-form').onsubmit=e=>{
 ...
 try{
  ...
  const method=$('edit-method').value,slot=$('edit-slot').value?Number($('edit-slot').value):null,amount=Number($('edit-amount').value),noCandy=method==='delibird'&&slot===6;
  ...
  if($('edit-context').checked){
   ...
   p=team[slot-1];if(!noCandy&&!p)throw Error('獲得先の編成を選んでください。');
   context={rulesVersion:3,team,actor:null,actorId:null,actorSlot:null,event:null,effectiveLevel:null};
   if(method!=='help'){
    const rawActor=editPool.find(p=>p.id===$('edit-actor').value);if(!rawActor?.profile)throw Error('個体情報のあるスキル個体を選んでください。');const actor=clone(rawActor);actor.profile.skillLevel=Math.min(C.skillCap(actor.species),actor.profile.skillLevel);
...
$('add-pokemon').onclick=()=>{if(!names.includes(selected)||!speciesEnabled(selected))return;if(selected==='ミュウ'&&state.pokemon.some(p=>p.species==='ミュウ')){notice('ミュウは登録済みです。');return;}const p={id:uid(),species:selected,nickname:$('nickname').value.trim(),profile:null,shardSkill:$('register-shard-skill').checked,registrationOrder:Math.max(-1,...state.pokemon.map(p=>p.registrationOrder))+1};if(commit({...state,pokemon:[...state.pokemon,p]},label(p)+'を登録しました。')){$('nickname').value='';$('register-shard-skill').checked=false;if(['ミュウ','デリバード'].includes(p.species)){showTab('settings');$('profile-select').value=p.id;editProfile();notice(label(p)+'を登録しました。');}}};
for(const id of ['period','summary-date','summary-actor','analysis-level','analysis-multiplier','analysis-amount','analysis-actor'])$(id).onchange=renderSummary;
$('history-toggle').onclick=()=>{showPastHistory=!showPastHistory;renderHistory();};
...
ShardUI.init();$('datetime').value=C.localInput();$('summary-date').value=C.gameDay(new Date());$('event-month').value=C.gameDay(new Date()).slice(0,7);load();selectEvent(null);renderCatalog();render();
...
// 上の日時欄と固定日付は同じ値を使い、日付変更時も時刻はそのまま残します。
function updateFixedRecordDate(){
 const root=$('fixed-record-date'),source=$('datetime'),input=$('fixed-record-date-input');
 root.hidden=activeTab!=='record'||source.getBoundingClientRect().bottom>0;
 const date=source.value.slice(0,10);if(!C.dateOnly(date))return;
 if(document.activeElement!==input)input.value=date;
 $('fixed-record-date-text').textContent=Number(date.slice(5,7))+'月'+Number(date.slice(8,10))+'日 ▼';
}
$('fixed-record-date-input').onchange=()=>{
 const date=$('fixed-record-date-input').value,source=$('datetime');
 if(!C.dateOnly(date))return;
 const time=source.value.slice(11)||C.localInput().slice(11);
 source.value=date+'T'+time;$('auto-now').checked=false;renderRecord();
};
$('fixed-record-date-input').onclick=e=>{if(typeof e.currentTarget.showPicker==='function'){try{e.currentTarget.showPicker();}catch(_){/* 未対応の場合は通常の入力操作を使います。 */}}};
window.addEventListener('scroll',updateFixedRecordDate,{passive:true});
window.addEventListener('resize',updateFixedRecordDate);
updateFixedRecordDate();
