...
function renderSummary(){
 ...
 calendar($('candy-calendar'),month,date=>{const b=button('',()=>{$('summary-date').value=date;$('period').value='day';renderSummary();},'day');b.append(el('strong',Number(date.slice(-2)),'day-number'));const values=el('span',undefined,'candy-values');for(const method of Object.keys(methods)){if(!enabled(method)||(method==='skill'&&state.settings.showOtherCandy===false))continue;const total=C.sum(calendarRs.filter(r=>r.method===method&&C.gameDay(r.datetime)===date));const name=method==='skill'?'その他':methods[method];const value=el('span',total.toLocaleString('ja-JP'),method);value.title=name+'：'+total.toLocaleString('ja-JP')+'個';value.setAttribute('aria-label',value.title);values.append(value);}b.append(values);b.setAttribute('aria-label',C.displayDate(date)+'のアメ');if(date===$('summary-date').value)b.classList.add('selected');return b;});
 $('calendar-other-legend').hidden=state.settings.showOtherCandy===false;
...
function renderSettings(){ $('show-other-candy').checked=state.settings.showOtherCandy!==false; $('enable-mew').checked=state.settings.mew;$('enable-delibird').checked=state.settings.delibird;const p=$('profile-select'),old=p.value;optionList(p,orderedPokemon().filter(p=>speciesEnabled(p.species)).map((p,i)=>({id:p.id,text:label(p)})),old);editProfile();$('profile-select').closest('.card').hidden=!state.pokemon.some(p=>speciesEnabled(p.species));$('event-month').closest('.card').hidden=!enabled('mew')&&!enabled('delibird');renderEvents();}
...
$('export').onclick=()=>{try{const data=blocked?JSON.parse(localStorage.getItem(KEY)):state;const url=URL.createObjectURL(new Blob([CandyBackup.encode(data)],{type:'text/csv;charset=utf-8'}));const a=el('a');a.href=url;const created=new Date();a.download='pokesleep-candy-'+C.localInput(created).replace(/[-:T]/g,'').replace(/^(\d{8})/,'$1_')+String(created.getSeconds()).padStart(2,'0')+'_'+String(created.getMilliseconds()).padStart(3,'0')+'.csv';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);notice('CSVバックアップを書き出しました。');}catch(e){notice('バックアップを書き出せませんでした：'+e.message);}};
...
$('show-other-candy').onchange=()=>commit({...state,settings:{...state.settings,showOtherCandy:$('show-other-candy').checked}},'表示設定を保存しました。');
