...
let noticeTimer;
function notice(s){const n=$('notice');n.textContent=s;n.classList.remove('shown');void n.offsetWidth;n.classList.add('shown');clearTimeout(noticeTimer);noticeTimer=setTimeout(()=>n.classList.remove('shown'),5000);}
...
function totals(parent,records,title){records=records.filter(visibleRecord);parent.replaceChildren(el('small',title),el('div',`${C.sum(records)}個`,'total'));for(const [method,name] of Object.entries(methods)){if(!enabled(method))continue;const rs=records.filter(r=>r.method===method);row(parent,name,method==='help'?`${C.sum(rs)}個`:`スキル${rs.length}回 · アメ${C.sum(rs)}個`,method);}}
...
function renderSummary(){
 ...
 $('slot-counts').replaceChildren();for(const [method,name] of Object.entries(methods)){if(!enabled(method))continue;const group=el('div',undefined,method);const source=rs.filter(r=>r.method===method);group.append(el('h4',`${name}${method==='help'?'':` · スキル${source.length}回`}`));for(let i=1;i<=5;i++){const xs=source.filter(r=>r.slot===i);row(group,position(i),`${xs.length}回 · ${C.sum(xs)}個`);}if(method==='delibird')row(group,'6 · スキルのみ',source.filter(r=>r.slot===6).length+'回');const unknown=source.filter(r=>r.slot===null);if(unknown.length)row(group,'獲得位置不明',unknown.length+'回 · '+C.sum(unknown)+'個');$('slot-counts').append(group);}
...
function renderAnalysis(){
 const period=C.range($('analysis-period').value,$('analysis-date').value||C.gameDay(new Date()));$('analysis-range').textContent=period?`${period[0]} 〜 ${C.addDays(period[1],-1)}`:'全期間';const records=state.records.filter(r=>visibleRecord(r)&&C.inRange(r,period));
 ...
 const wrap=el('div',undefined,'table-scroll'),table=el('table'),head=el('tr');head.append(el('th','スキル位置 ↓／獲得先 →'));for(let i=1;i<=(method==='mew'?5:6);i++)head.append(el('th',i===6?'アメなし':String(i)));head.append(el('th','スキル回数'));table.append(head);
...
function renderHistory(){const root=$('history-list');root.replaceChildren();for(const r of [...state.records].filter(visibleRecord).reverse().sort((a,b)=>Date.parse(b.datetime)-Date.parse(a.datetime))){
...
$('analysis-period').onchange=()=>renderAnalysis();$('analysis-date').onchange=()=>renderAnalysis();
...
$('analysis-date').value=C.gameDay(new Date());$('datetime').value=C.localInput();$('summary-date').value=C.gameDay(new Date());$('candy-month').value=C.gameDay(new Date()).slice(0,7);$('event-month').value=$('candy-month').value;load();selectEvent(null);renderCatalog();render();
